...
- **即時互動指令**：支援在搜尋過程中隨時切換檔案、調整大小寫敏感度、切換 Regex / Literal 搜尋模式、設定前後上下文行數（Context），以及直接列印指定行數範圍的內容。
...
| `:p <起始>,<結束> [關鍵字]` | `:print <起始>,<結束> [關鍵字]` | 直接列印檔案中指定行數範圍的內容，每次最多顯示 100 行；可選填關鍵字，匹配文字將以高亮標註（例如：`:p 1,20` 或 `:p 1,20 error`） |
